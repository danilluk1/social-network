...
import (
	...
	"github.com/danilluk1/social-network/libs/go/pkg/otel/tracing/attribute"
...
func (server *GAPI) CreateUser(ctx context.Context, req *auth.CreateUserRequest) (*auth.CreateUserResponse, error) {
	...
	server.authMetrics.SignUpRequests.Add(ctx, 1, grpcMetricsAttr)

	command, err := createUser.NewCreateUser()
